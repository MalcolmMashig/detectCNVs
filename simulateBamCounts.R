...
simulateBamCounts <- function(
  ...
  if (!use_pkg_data) {
    ...
    new <- ExomeCount %>% 
      mutate(
        Exome1 = round(Exome1 + rnorm(n, 10, 5)),
        Exome2 = round(Exome2 + rnorm(n, 10, 5)),
        Exome3 = round(Exome3 + rnorm(n, 10, 5)),
        Exome4 = round(Exome4 + rnorm(n, 10, 5)),
...
computeAcc <- function(calls, truth, rounded = 6) {
  sens <- calls %>% 
    mutate(start = round(start, -rounded), end = round(end, -rounded)) %>% 
    right_join(
      truth %>% 
        mutate(start = round(start, -rounded),
               end = round(end, -rounded))
    ) %>% 
    filter(CNV == type) %>% 
  ...
  dels <- sens %>% 
    filter(CNV == "deletion")
  delsens <- nrow(dels) / (nrow(truth %>% filter(CNV == "deletion")))
  delspec = nrow(dels) / (nrow(calls %>% filter(type == "deletion")))
  dups <- sens %>% 
    filter(CNV == "duplication")
  dupsens <- nrow(dups) / (nrow(truth %>% filter(CNV == "duplication")))
...
sim <- simulateBamCounts(use_pkg_data = TRUE)
ExomeCount.dafr <- sim$counts
truth <- sim$truth
# data("ExomeCount")
# ExomeCount.dafr <- data.frame(ExomeCount)
...
# calls <- calls %>%
#   select(start, end, CNV = type) %>%
#   write_csv("ground-truth.csv")
