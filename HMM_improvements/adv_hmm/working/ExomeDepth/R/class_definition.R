...
setMethod("CallCNVs", "ExomeDepth", function( x, chromosome, start, end, name, transition.probability, expected.CNV.length) {
  ...
  for (chrom in my.chromosomes)  {  ##run Viterbi separately for each chromosome
    ...
    loc.likelihood <- rbind(c(-Inf, 0, -Inf, -Inf, -Inf, Inf), x@likelihood[good.pos, c(2, 1, 3, 2, 1, 3)],c(-100,0,-100, -100, -100, -100)) ##update from Anna Fowler add a dummy exon so that we start at cn = 2 (normal) and a dummy exon at the end of the chromosome as well so that it ends at cn=2; for some reason I had to use -100 instead of -Inf
  ...
  ################################ Now make it look better, add relevant info
    if (nrow(my.calls$calls) > 0) {
      ...
      my.calls$calls$type <- c('deletion', 'duplication', 'active', 'deletion', 'duplication')[ my.calls$calls$type ]
